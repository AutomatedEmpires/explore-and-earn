...
/**
 * Error boundary for the seeker message thread. Offers a retry and a path
 * back to the messages list. Reports to Sentry on mount.
 */
export default function MessageThreadError({
	...
	useEffect(() => {
		reportError(error, { route: "seeker-message-thread" });
	...
	return (
		<section className={styles.wrap} role="alert">
			...
			<Link className={styles.action} href="/messages">
				All messages
			</Link>
		</section>
	);
