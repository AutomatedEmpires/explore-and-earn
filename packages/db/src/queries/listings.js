 ...
 */
export async function getSwipeBatch(clerkToken, clerkUserId, excludeIds, cursor) {
    ...
    if (error)
        throw new Error(`getSwipeBatch: ${error.message}`);
    return (data ?? []).map(toListingRow);
}
/**
 ...
 * public read).
 */
export async function getLiveListingsWithCoords(clerkToken) {
    ...
    const { data, error } = await db
        ...
        .order("published_at", { ascending: false });
    if (error)
        throw new Error(`getLiveListingsWithCoords: ${error.message}`);
    return (data ?? []).map(toListingRow);
}
 ...
 */
export async function getListingDetailPublic(listingId) {
    const db = anonClient();
    const { data, error } = await db
        .from("listings")
        ...
        .maybeSingle();
    if (error)
        throw new Error(`getListingDetailPublic: ${error.message}`);
    if (!data)
        return null;
    const row = data;
    ...
    const host = hostRow
        ? {
            ...
            companyName: typeof hostRow.company_name === "string" ? hostRow.company_name : "",
            photoUrl: typeof hostRow.photo_url === "string" ? hostRow.photo_url : null,
            about: typeof hostRow.about === "string" ? hostRow.about : null,
            primaryLocationName: typeof hostRow.primary_location_name === "string"
                ? hostRow.primary_location_name
                : null,
            attestationStatus: typeof hostRow.attestation_status === "string"
    ...
    return {
        ...
        compensationCurrency: typeof row.compensation_currency === "string"
            ...
            : "USD",
        timelineSummary: typeof row.timeline_summary === "string" ? row.timeline_summary : null,
 ...
 */
async function resolveSeekerProfileIdForListings(clerkToken, clerkUserId) {
    ...
    const { data, error } = await db
        ...
        .maybeSingle();
    if (error)
        throw new Error(`resolveSeekerProfileIdForListings: ${error.message}`);
 ...
 */
export async function hasApplied(clerkToken, clerkUserId, listingId) {
    const seekerProfileId = await resolveSeekerProfileIdForListings(clerkToken, clerkUserId);
    if (!seekerProfileId)
        return false;
    const db = authedClient(clerkToken);
    const { data, error } = await db
        ...
        .maybeSingle();
    if (error)
        throw new Error(`hasApplied: ${error.message}`);
    return Boolean(data);
}
/**
 ...
 * `clerkUserId` must come from auth().userId.
 */
export async function hasSaved(clerkToken, clerkUserId, listingId) {
    const seekerProfileId = await resolveSeekerProfileIdForListings(clerkToken, clerkUserId);
    if (!seekerProfileId)
        return false;
    const db = authedClient(clerkToken);
    const { data, error } = await db
        ...
        .maybeSingle();
    if (error)
        throw new Error(`hasSaved: ${error.message}`);
    return Boolean(data);
}
/**
 ...
 */
export async function getHostIdsWithLiveListings() {
    const db = anonClient();
    const { data, error } = await db
        .from("listings")
        ...
        .eq("status", "live");
    if (error)
        throw new Error(`getHostIdsWithLiveListings: ${error.message}`);
    const ids = new Set();
    for (const raw of (data ?? [])) {
        if (raw.host_profile_id)
            ids.add(raw.host_profile_id);
    }
    return [...ids];
}
