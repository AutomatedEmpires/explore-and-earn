...
export async function applyToListing(clerkToken, clerkUserId, listingId, coverMessage) {
    ...
    const authed = authedClient(clerkToken);
 ...
 */
export async function getApplicationsForSeekerWithListings(clerkToken, clerkUserId) {
    const seekerProfileId = await resolveSeekerProfileId(clerkToken, clerkUserId);
    if (!seekerProfileId)
        return [];
    const untyped = authedClient(clerkToken);
 ...
 */
export async function getApplicationById(clerkToken, clerkUserId, applicationId) {
    const seekerProfileId = await resolveSeekerProfileId(clerkToken, clerkUserId);
    if (!seekerProfileId)
        return null;
    const untyped = authedClient(clerkToken);
