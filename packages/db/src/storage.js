...
async function uploadToBucket(token, bucket, path, file) {
 ...
 */
export async function uploadListingMedia(token, hostProfileId, file, slot) {
 ...
 * its public URL.
 */
export async function uploadProfilePhoto(token, ownerId, file, ownerType) {
    return uploadToBucket(token, PROFILE_PHOTOS_BUCKET, `${ownerType}/${ownerId}`, file);
}
/** Delete a single object from a bucket by path. */
export async function deleteStorageObject(token, bucket, path) {
    const client = authedClient(token);
