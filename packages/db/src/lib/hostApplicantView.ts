...
/**
 * Ids per call to public.get_host_applicant_display_names.
 *
 * The database states the same number as a hard bound and raises 54000
 * (program_limit_exceeded) above it; this constant is what the callers chunk on
 * so they stay under it. Two statements of one number, so
 * tests/hostApplicantView.test.ts parses the bound out of migration 084 and
 * fails if they disagree.
 */
export const HOST_APPLICANT_NAME_BATCH = 200;
...
/**
 * A display-name lookup either answered, or did not.
 *
 * WHY THE NAME PATH DOES NOT THROW AND THE RESUME PATH DOES
 *
 * unwrapBridgeRows above is the right contract for the applicant DETAIL read:
 * the resume IS that page, and a resume that silently comes back empty is
 * precisely the defect 084 removed, so a fault there must stop the page.
 *
 * Display names are not that. They decorate four list surfaces — the applicant
 * pipeline, the listing's applicant table, and both host message lists — whose
 * real content (applications, listings, threads, messages) is loaded elsewhere
 * and is unaffected by a name lookup failing. Letting the RPC throw there turned
 * a decoration fault into a 500 on the host's whole pipeline, and opened a
 * deploy-order window: application code reaches production before the migration
 * is applied, and for those minutes get_host_applicant_display_names does not
 * exist, so every one of those pages would have been down.
 *
 * The wrong repair is `catch { return new Map() }` — that rebuilds the original
 * bug, because an unresolvable name and an unentitled one would both render the
 * caller's placeholder and nothing would say which happened. So the failure is
 * kept, as data:
 *
 *   status "resolved"    -> the map is the answer. An id absent from it is an id
 *                           the caller is not entitled to, or a seeker with no
 *                           name. The caller's own placeholder is correct.
 *   status "unavailable" -> nothing was resolved and nothing is known. Callers
 *                           must say so (resolveSeekerName does), never fall
 *                           back to a placeholder that reads like a real answer.
 */
export type SeekerNameLookup =
  | { readonly status: "resolved"; readonly names: ReadonlyMap<string, string> }
  | { readonly status: "unavailable"; readonly reason: string };

/**
 * What a surface shows where a name would go when the lookup itself failed.
 * Deliberately not "Seeker" or any other plausible stand-in: the host is being
 * told the system could not answer, not being shown an anonymous applicant.
 */
export const SEEKER_NAME_UNAVAILABLE = "Name unavailable";

/** A lookup that resolved nothing because it was never asked. */
export function emptySeekerNameLookup(): SeekerNameLookup {
  return { status: "resolved", names: new Map() };
}

/**
 * Decode a batch name RPC response into a lookup, converting a fault into
 * `unavailable` rather than into an exception or into silence.
 *
 * The fault is still loud where loudness belongs — the reason travels with the
 * value and the caller logs it — but it no longer decides whether the page
 * renders.
 */
export function readSeekerNameLookup(
  label: string,
  result: { data: unknown; error: { message?: string } | null } | null | undefined,
): SeekerNameLookup {
  try {
    const rows = unwrapBridgeRows(label, result);
    return { status: "resolved", names: mapHostApplicantDisplayNames(rows) };
  } catch (caught) {
    return {
      status: "unavailable",
      reason: caught instanceof Error ? caught.message : `${label}: unknown lookup failure`,
    };
  }
}

/**
 * Merge a further page of results into a lookup. One failed chunk makes the
 * whole lookup unavailable: a partially-resolved map would render some real
 * names and some placeholders, which is the ambiguity this type exists to
 * prevent.
 */
export function mergeSeekerNameLookups(
  first: SeekerNameLookup,
  second: SeekerNameLookup,
): SeekerNameLookup {
  if (first.status === "unavailable") return first;
  if (second.status === "unavailable") return second;
  const merged = new Map(first.names);
  for (const [id, name] of second.names) merged.set(id, name);
  return { status: "resolved", names: merged };
}

/**
 * The single place a lookup becomes a label. Every host surface goes through
 * this, so no surface can quietly reintroduce "a broken read looks like an
 * anonymous applicant".
 */
export function resolveSeekerName(
  lookup: SeekerNameLookup,
  seekerProfileId: string,
  whenUnnamed: string,
): string {
  if (lookup.status === "unavailable") return SEEKER_NAME_UNAVAILABLE;
  return lookup.names.get(seekerProfileId) ?? whenUnnamed;
}

/**
 * One seeker's name, for the surfaces that resolve exactly one. `null` means
 * resolved-and-nameless; the unavailable case is reported separately by the
 * caller so it cannot be mistaken for "this seeker has no name".
 */
export function singleSeekerName(
  lookup: SeekerNameLookup,
  seekerProfileId: string,
): string | null {
  if (lookup.status === "unavailable") return null;
  return lookup.names.get(seekerProfileId) ?? null;
}
