 ...
 *     allowance, which handed every non-paying host a free live listing. The
 *     founder's decision of 2026-07-26 is that there is no free tier, 083's
 *     private.plan_listing_allowance() returns 0 for 'none', and the database is
 *     the thing that actually refuses. A cap check that quotes 1 where the
 *     database enforces 0 protects nothing; it only picks which of the two
 *     numbers the host is lied to with.
 *
