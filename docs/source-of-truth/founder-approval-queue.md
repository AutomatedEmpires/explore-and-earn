...
| A-MIG-022-EXPIRY | Apply migration `022_listing_expiry.sql` (adds `listings.expires_at` + auto-expiry insert trigger + backfill)? | (Schema / migration apply) | Additive only: `ADD COLUMN IF NOT EXISTS` + index + `BEFORE INSERT` default trigger; **no RLS change, no data loss**. Backfills `expires_at` for existing live listings as `begins_at + 90 days`. Pairs with the `/api/cron/expire-listings` route (Bearer `CRON_SECRET`). | Apply in the next migration window; additive and reversible (drop column). | Waiting |
