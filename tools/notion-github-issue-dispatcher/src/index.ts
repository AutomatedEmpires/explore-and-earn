 ...
 *
 * This is step 2 of the Explore&Earn handoff protocol (docs/agents/handoff-protocol.md):
 * a Notion task that is `Ready for Engineering` is dispatched into a GitHub issue,
 * and the created issue URL is written back into the Notion source page so the
 * baton is visible to the next agent.
 *
...
worker.tool("dispatchTaskToGithub", {
	...
	description:
		"Create a GitHub issue from a Notion handoff (Ready for Engineering) using the Explore&Earn handoff-protocol shape, then write the created issue URL back to the Notion source page. This creates a real issue — confirm before running.",
	schema: j.object({
		notionSourceUrl: j
			.string()
		...
		const res = await fetch(
			`https://api.github.com/repos/${owner}/${repo}/issues`,
			{
				...
				headers: {
					...
					"User-Agent": "explore-and-earn-issue-dispatcher",
				},
