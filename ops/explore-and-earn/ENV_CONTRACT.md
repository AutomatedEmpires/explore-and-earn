...
| `UNSPLASH_ACCESS_KEY` | Own Unsplash app (free tier) | None at runtime — read ONLY by `scripts/seed-site-photos.mjs` | no / no / no | Never needed by the app; supply it locally or via Doppler when seeding site photography | Not provisioned | Image delivery is Supabase Storage; there is no image CDN and no CDN credential to hold (see `docs/design/site-photos.md`) |
