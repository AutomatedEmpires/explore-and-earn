...
--     inside public.host_can_view_seeker, which every projection here is gated
--     on, so a soft-deleted account (079) stops being readable -- profile,
--     names and resume alike -- without the host being able to read the
--     timestamp itself.
--
...
--
-- SOFT DELETE IS DECIDED HERE, ONCE.
--
-- 079 soft-deletes a seeker by stamping deleted_at. An earlier draft of this
-- migration applied that filter in each projection, and only two of the five
-- carried it: the three resume functions selected from the resume tables, which
-- have no deleted_at of their own, so a soft-deleted seeker's whole work history
-- stayed readable by every entitled host. A per-projection filter is a rule that
-- must be remembered five times and was remembered twice. It is a conjunct of
-- the entitlement itself instead, so a sixth projection added later inherits it
-- and cannot forget it. The projections that select seeker_profiles directly
-- keep their own `deleted_at is null` as well -- redundant, not contradictory,
-- and it keeps each of those queries readable on its own.
-- ---------------------------------------------------------------------------
...
--
-- OVER THE BOUND IS AN ERROR, NOT AN EMPTY ANSWER.
--
-- The first version of this expressed the bound as a WHERE conjunct, so 201 ids
-- returned zero rows -- indistinguishable, to the caller, from 201 seekers none
-- of whom it is entitled to. That is the same "denied looks like absent"
-- confusion the whole migration exists to remove, rebuilt at the edge of the
-- contract: a caller that stopped chunking would render every applicant as a
-- placeholder and nothing anywhere would say why. Over the bound now raises
-- 54000 (program_limit_exceeded), which the client surfaces as a fault. Ids
-- the caller is not entitled to are still simply absent -- that is the answer
-- to a well-formed question, and it stays quiet.
--
-- The bound is stated once more, in TypeScript, as HOST_APPLICANT_NAME_BATCH in
-- packages/db/src/lib/hostApplicantView.ts, which is what the callers chunk on.
-- packages/db/tests/hostApplicantView.test.ts parses the number out of this file
...
--
-- THIS BLOCK IS NOT THE ONLY PLACE THESE CLAIMS ARE CHECKED, AND MUST NOT BE.
--
-- A DO block in a migration runs exactly once, when the migration is applied.
-- It proves 084 was correct on the day it landed; it says nothing about the
-- schema afterwards, because a later migration that redefines
-- host_can_view_seeker, or re-grants one of these functions to anon, does not
-- re-run it. Every assertion below is therefore also carried by
-- tools/db-assert/sql/assert_host_applicant_bridge.sql, which runs on every
-- pull request against the database rebuilt from 001, and which covers the
-- widening cases this block cannot see.
-- ---------------------------------------------------------------------------
do $$
declare
...
begin
  ...
  begin
    ...
    -- ---- over the batch bound: an error, never an empty answer ----
    set local role authenticated;
    set local request.jwt.claims = '{"sub":"user_0084hostA"}';
    begin
      select count(*) into v_n
        from public.get_host_applicant_display_names(
          array(select gen_random_uuid() from generate_series(1, 201))::uuid[]
        );
      v_failures := v_failures || format(
        ' [over-limit batch returned %s rows instead of raising]', v_n);
    exception
      when program_limit_exceeded then null;
    end;

    -- ---- a soft-deleted seeker (079) disappears from EVERY projection ----
    -- Checked last, because it destroys the fixture the checks above rely on.
    set local role none;
    ...
    if v_n <> 0 then
      v_failures := v_failures || format(
        ' [soft-deleted seeker profile still readable: %s rows]', v_n);
    end if;
    ...
    if v_n <> 0 then
      v_failures := v_failures || format(
        ' [soft-deleted seeker name still readable: %s rows]', v_n);
    end if;
    ...
    if v_n <> 0 then
      v_failures := v_failures || format(
        ' [soft-deleted seeker resume still readable: %s rows]', v_n);
    end if;
  ...
  raise notice 'host_applicant_read_bridge: applicant readable, unrelated seeker not, soft-deleted seeker not, over-limit batch raised, anon refused';
end
$$;
